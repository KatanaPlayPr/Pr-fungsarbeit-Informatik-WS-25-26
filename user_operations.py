...
def createUser(currentUserName):
    if currentUserName == "Admin":
        ...
        if userNames.count(newUserName) == 0:
            ...
            print("Benutzer mit solche Benutzername schon existiert. Überprüffen Sie die Liste der Nutzer und versuchen Sie es erneut.", end = "\n\n")
    else:
...
def changePassword(currentUserName):
    ...
    if oldPassword == passwords[index] and newPassword == newPassword1:
        ...
        rewriteUsersData(userNames, passwords)
        print("Passwort erfolgreich geändert!", end = "\n\n")
    else:
    ...
    if currentUserName == "Admin":
        ...
        if userNameToChange in userNames and userNameToChange != "Admin":
            ...
            if newUserName not in userNames:
                ...
                rewriteUsersData(userNames, passwords)
                print(f"Benutzername {userNameToChange} erfolgreich zu {newUserName} geändert!", end = "\n\n")
            else:
                print("Benutzer mit solche Benutzername schon existiert. Überprüffen Sie die Liste der Nutzer und versuchen Sie es erneut.", end = "\n\n")
        elif userNameToChange == "Admin":
            print("Der Admin-Benutzername kann nicht geändert werden.", end = "\n\n")
        else:
            print("Benutzername nicht gefunden.", end = "\n\n")
    else:
        print("Sie haben keine Berechtigung, Benutzernamen zu ändern.", end = "\n\n")
    ...
    while condition:
        ...
        try:
            if choice == "1":
                ...
                print("Logout erfolgreich!", end = "\n\n")
                condition = False
            else:
                print("Ungültige Eingabe. Bitte geben Sie eine Zahl zwischen 1 und 6 ein.", end = "\n\n")
        except Exception as e:
            print(f"Ein Fehler ist aufgetreten: {e}. Bitte versuchen Sie es erneut oder wenden Sie sich an einen Administrator.", end = "\n\n")
